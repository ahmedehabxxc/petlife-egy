...
const VetDashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const vetResponse = await api.get("/Veterinarians/me");
        const vetData = vetResponse.data || {};
        setIsOnline(Boolean(vetData.isOnline ?? vetData.IsOnline ?? false));
        ...
        const active = rows.filter((r: any) => ["accepted", "in_progress"].includes(r.status ?? r.Status)).length;
        ...
        const recent = [...rows]
          .filter((r: any) => ["accepted", "in_progress", "completed"].includes(r.status ?? r.Status))
  ...
  const handleAvailabilityToggle = async () => {
    ...
    try {
      await api.put("/Veterinarians/me/availability", {
        isOnline: nextValue,
      });
      setIsOnline(nextValue);
    } catch {
      // ignore for now to keep existing UX quiet
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Badge
            variant="outline"
            className={isOnline ? "bg-success/10 text-success border-success/20" : "bg-muted text-muted-foreground"}
          >
          ...
          >
            {availabilityLoading ? "Saving..." : isOnline ? "Go Offline" : "Go Online"}
          </Button>
        </div>
      </div>
